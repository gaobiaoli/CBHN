...
class SwinHN(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=6, embed_dim=96, depths=(2, 6, 4),
                 num_heads=(3, 12, 24), window_size=8, mlp_ratio=4., qkv_bias=True, 
                 drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, num_classes=8, ape=False, 
        ...
        self.blocks_token_only = nn.ModuleList([
            LayerScale_Block_CA(
                ...
                out_dim=int(self.embed_dim * 2 ** blks_list[max(i, len(blks_list) - 1)]), num_heads=num_heads[i],
                mlp_ratio=self.mlp_ratio, qkv_bias=qkv_bias, qk_scale=None,
    ...
    def forward(self, x):
        ...
        x = self.head(x)  # Linear regression to output 8 numbers
        return x
