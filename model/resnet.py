...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        fea_x4 = self.layer1(x)
        fea_x8 = self.layer2(fea_x4)
        fea_x16 = self.layer3(fea_x8)
        fea_x32 = self.layer4(fea_x16)

        # x = self.avgpool(x)
        # x = torch.flatten(x, 1)
        # x = self.fc(x)

        return [fea_x4,fea_x8,fea_x16,fea_x32]

    def forward(self, x: Tensor) -> Tensor:
        return self._forward_impl(x)
    
def resnet18(pretrain=True,**kwargs: Any):
    return ResNet(BasicBlock, [2, 2, 2, 2],**kwargs)

def resnet34(pretrain=True,**kwargs: Any):
    return ResNet(BasicBlock, [3, 4, 6, 3],**kwargs)

def simplenet(**kwargs: Any):
